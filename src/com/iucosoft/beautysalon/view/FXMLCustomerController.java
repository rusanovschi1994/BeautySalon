...
public class FXMLCustomerController implements Initializable, FormularIntf<Customer> {
    ...
    CustomerFileServiceIntf customerFileService;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            customerFileService = new CustomerFileServiceImpl();
            refreshList();
            clearForm();
    ...
    private void showSearchResult(List<Customer> customersName) {
        ...
        if (customersName.size() == 0) {
            ...
            showMessage(Alert.AlertType.WARNING, "Căutare",
                    ...
                    + "Te rog, alege clientul manual din lista de mai jos.");
    ...
    private void handleExcelExportAction(ActionEvent event) {
        ...
        String fileName = "clienti_" + LocalDateTime.now().
                format(DateTimeFormatter.ISO_DATE) + ".xlsx";

        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Export clienți EXCEL");
        alert.setHeaderText("ATENȚIE!");
        ...
        if (result.get() == yes) {
            try {
                customerFileService.scrie(oL, fileName);
                showMessage(Alert.AlertType.INFORMATION,
                        "Clienți",
                        "MESAJ INFORMATIONAL",
    ...
    private void handlePdfExportAction(ActionEvent event) {
        ...
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Export clienți PDF");
        alert.setHeaderText("ATENȚIE!");
        alert.setContentText("Vrei să exporți datele clienților?");
        ...
        if (result.get() == yes) {
            try {
                customerFileService.creaza(oL, fileName);
                showMessage(Alert.AlertType.INFORMATION,
                        "Clienți",
                        "MESAJ INFORMATIONAL",
                        "Informația a fost exportată cu succes în fișierul, " + fileName);
