...
import com.iucosoft.beautysalon.fileservice.fileserviceDao.RegistrationFileServiceImpl;
...
public class FXMLRegistrationController implements Initializable, FormularIntf<Operation> {
    ...
    @FXML
    private TableColumn<Operation, Customer> tableCustomers;
    @FXML
    private TableColumn<Operation, Customer> tableUsers;
    ...
    RegistrationFileServiceIntf registrationFileService;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            registrationFileService = new RegistrationFileServiceImpl();
    ...
    private void handleExcelExportAction(ActionEvent event) {
        String fileName = "inregistrari_" + LocalDateTime.now().
                format(DateTimeFormatter.ISO_DATE) + ".xlsx";

        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Export înregistrări");
        alert.setHeaderText("ATENȚIE!");
        alert.setContentText("Vrei să exporți înregistrările?");
        ...
        if (result.get() == yes) {
            try {
                registrationFileService.scrie(obsL, fileName);
                showMessage(Alert.AlertType.INFORMATION,
                        "Înregistrări",
                        "MESAJ INFORMATIONAL",
    ...
    @FXML
    private void handleXmlExportAction(ActionEvent event) {
    }

    @FXML
    private void handlePdfExportAction(ActionEvent event) {
        String fileName = "înregistrări_" + LocalDateTime.now().
                format(DateTimeFormatter.ISO_DATE) + ".pdf";

        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Export înregistrări în PDF");
        alert.setHeaderText("ATENȚIE!");
        alert.setContentText("Vrei să exporți datele înregistrărilor?");
        ...
        if (result.get() == yes) {
            try {
                registrationFileService.creaza(obsL, fileName);
                showMessage(Alert.AlertType.INFORMATION,
                        "Înregistrări",
                        "MESAJ INFORMATIONAL",
                        "Informația a fost exportată cu succes în fișierul, " + fileName);
...
//    }
    ...
    public void clearForm() {
        ...
        tfOperationR.setValue(OperationType.None);
...
//              Stream<Customer> items = tfCustomerName.getItems().stream();
    ...
    private void cellOperationsValueFactory() {
        ...
        tableCustomers.setCellValueFactory(new PropertyValueFactory<Operation, Customer>("customer"));
        tableUsers.setCellValueFactory(new PropertyValueFactory<Operation, Customer>("user"));
